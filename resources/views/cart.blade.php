...
<div class="container" style="margin-top: 70px;">
    <p>
        <b style="font-size: 17px;">Thông tin về chuyến bay đã đặt</b>
    </p>
    <table class="table table-bordered">
        ...
        @if(isset($tickets))
            @for($i = 0; $i < count($tickets); $i++)
                <tr>
                    <td>{{$tickets[$i]->ChuyenBay->tenhang}}</td>
                    <td>{{$tickets[$i]->ChuyenBay->sohieu}}</td>
                    <td>
                        <?php
                            $idLoTrinh =  $tickets[$i]->ChuyenBay->idLoTrinh;
                            $lotrinh = DB::table('lotrinh')->where('idLoTrinh', '=', $idLoTrinh)->get();
                            echo $lotrinh[0]->start;
                        ?>
                    </td>
                    <td>{{$lotrinh[0]->end}}</td>
                    <td>{{date("d-m-Y", strtotime($tickets[$i]->ChuyenBay->ngaybay))}}</td>
                    <td>{{$tickets[$i]->ChuyenBay->giobay}}</td>
                    <td>{{$tickets[$i]->quantity}}</td>
                    <td>{{$tickets[$i]->total}} $</td>
                    <td>{{$tickets[$i]->status}}</td>
                    <td style="text-align: center;">
                        @if($tickets[$i]->status == 'No')
                            <form action="{{route('ticketpayment')}}" method="post">
                            @csrf
                                <input type="hidden" value="{{$tickets[$i]->orderID}}" name="ticketOrdered">
                                <input type="hidden" value="{{$tickets[$i]->total}}" name="ticketPrice">
    ...
    <p style="margin-top: 40px;">
        <b style="font-size: 17px;">Thông tin về khách sạn đã đặt</b>
    </p>
    <table class="table table-bordered">
        <tr class="table-success">
            <th>Tên khách sạn</th>
        ...
        @if(isset($rooms))
            @for($i = 0; $i < count($rooms); $i++)
                <tr>
                    <td>{{$rooms[$i]->Hotel->hotelName}}</td>
                    <td>{{$rooms[$i]->Hotel->address}}</td>
                    <td>{{$rooms[$i]->soPhong}}</td>
                    <td>{{date("d-m-Y", strtotime($rooms[$i]->ngayNhan))}}</td>
                    <td>{{date("d-m-Y", strtotime($rooms[$i]->ngayTra))}}</td>
                    <td>{{$rooms[$i]->total}}$</td>
                    <td>{{$rooms[$i]->status}}</td>
                    <td>
                        @if($rooms[$i]->status == 'NO')
                            <form action="{{route('hotelpayment')}}" method="post">
                            @csrf
                                <input type="hidden" value="{{$rooms[$i]->orderID}}" name="hotelBooking">
                                <input type="hidden" value="{{$rooms[$i]->total}}" name="hotelPrice">
                                <button type="submit" class="btn btn-light">
                                    <i class="fab fa-cc-mastercard"></i>
                                </button>
                            </form>
